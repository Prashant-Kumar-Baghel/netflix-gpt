...
const useTrendingMovies=()=>{
  ...
  const fetchData=async ()=>{
    ...
    //we have two ways to use UpComingMovies list data 1) using state variable i.e create state variable and update that state variable when we get data using fetch api and  return that state variable then we can access that data 2) using redux store that is when we get UpComingMovies data just store that data inside redux store and we can use that data from store. now when we use redux store then we don't need to create state variable . using redux components don't need to manage their own state so put the data inside central store and retrive anywhere we want.
    dispatch(addTrendingMovies(data?.results));//create addUpComingMovies reducer inside moviesSlice.
